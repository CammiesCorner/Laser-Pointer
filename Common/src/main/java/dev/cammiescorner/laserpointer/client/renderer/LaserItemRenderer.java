...
public class LaserItemRenderer implements BuiltinItemRendererRegistry.DynamicItemRenderer, IdentifiableResourceReloadListener {
	...
	public LaserItemRenderer(ResourceLocation itemId) {
	...
	public CompletableFuture<Void> reload(PreparableReloadListener.PreparationBarrier synchronizer, ResourceManager manager, ProfilerFiller prepareProfiler, ProfilerFiller applyProfiler, Executor prepareExecutor, Executor applyExecutor) {
		return synchronizer.wait(Unit.INSTANCE).thenRunAsync(() -> {
			...
			inventoryItemModel = client.getModelManager().getModel(new ModelResourceLocation(LaserPointer.id(itemId.getPath() + "_gui"), "inventory"));
			worldItemModel = client.getModelManager().getModel(new ModelResourceLocation(LaserPointer.id(itemId.getPath() + "_handheld"), "inventory"));
			applyProfiler.pop();
